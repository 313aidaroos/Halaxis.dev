...
export async function runAgentJob(jobId?: string, ownerId?: string) {
  ...
  try {
    ...
    const category = /credit|balance|billing|insufficient_quota/i.test(detail) ? "AI provider credits or billing need attention." : status === 401 || status === 403 ? `AI provider credentials were rejected (${status}).` : status === 404 ? "The configured AI model is unavailable (404)." : status === 429 ? "The AI provider quota or rate limit was reached (429)." : status === 400 ? `The AI provider rejected its request (400; ${/max_tokens|completion_tokens/i.test(detail) ? "token parameter" : /model/i.test(detail) ? "model configuration" : "provider configuration"}).` : error instanceof SyntaxError || (error instanceof Error && error.name === "ZodError") ? "The AI response did not match the required brief format." : error instanceof Error && error.message.includes("venture context") ? "The venture context could not be loaded." : "Agent processing or persistence failed.";
