...
2. **`src/lib/apixis-wallet.ts` and `src/lib/apixis-login.ts` are copies** of `ApixisWallet/sdk/apixis-wallet.ts` and `sdk/apixis-login-next.ts` (SDK v3). Don't fork or edit them here. Change them in ApixisWallet, then copy them over.
...
| Piece | Where |
|---|---|
| Sign in with Apixis | `/auth/apixis/start?next=…` → Wallet → `/auth/apixis/callback`, plus the button in `src/components/SignInWithApixis.tsx` on the login page |
| Shared balance + Buy Ixis link | `GET /api/wallet/balance` (the person's one Wallet balance, plus a `buy` URL that returns here) and `src/components/ApixisWalletChip.tsx` |
...
## Halaxis note
Halaxis has no Wallet SKUs yet, so "Buy Ixis" opens the Wallet itself (product=wallet). Add SKUs to ApixisWallet `lib/catalog.ts` (Awad approves prices) before selling Halaxis plans for Ixis.
